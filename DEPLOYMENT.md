...
**자동배포가 꺼져 있음** (2026-09-09 확인: Railway GitHub App 연결/권한 문제로 `canEnable: false`
상태). `git push`만으로는 배포가 안 됨. push 후 Railway 대시보드 → topclass-mirae 프로젝트 →
`topclass-mirae` 서비스를 열면 좌측에 "Apply N changes" 배너와 보라색 **Deploy** 버튼이 뜨는데,
이걸 직접 눌러줘야 새 커밋이 빌드/배포됨. (GitHub 계정 재연결로 자동배포를 살릴 수도 있지만
로그인/권한 승인이 필요해서 사람이 직접 해야 함 — 안 급하면 그냥 수동 Deploy로 충분.)
...
- ~~주소 입력 시 좌표(위도/경도) 자동 변환~~ → 2026-09-09 구현 완료 (`src/lib/geocode.ts`,
  카카오 로컬 API). `KAKAO_REST_API_KEY` 환경변수가 있어야 실제로 동작함 (없으면 조용히
  스킵되고 기존처럼 수동 입력만 됨). 카카오 개발자센터 → 앱 "탑클래스 미래"(id 1571287) →
  앱 키 → REST API 키.
- ~~네이버부동산 링크 자동 수집~~ → 2026-09-09 구현 완료 (`src/lib/naver-link.ts`). 정확한
  매물 링크가 아니라 주소로 네이버부동산 검색결과 페이지에 연결되는 링크를 자동 생성함
  (링크를 직접 입력 안 했을 때만 고객 지도 화면에 폴백으로 뜸).
